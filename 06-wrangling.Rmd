...
<!--- recortar chunk abaixo e explicar o passo a passo --->
...
<!--- ao inves de comparar as funcoes dos pacotes, só falar da tokens() --->

A função `tokens()`, é similar a função `unnest_tokens` do pacote `tidytext`. Ambas segmentam o texto, sua principal diferença é que ele não estrutura em forma de data frame com linhas e colunas o objeto tokenizado. Usaremos o corpus gerado do item anterior. Esta função somente remove espaços em branco e demais separadores de texto automáticamente, para remoção de pontuação e números é necessário escrever no código. 
