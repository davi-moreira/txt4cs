...
A análise de conteúdo só é possível através da transformação do texto bruto em estruturas de dados convenientes para o uso em pacotes, ou seja, transformamos o texto em dado analisável. Esta etapa é fundamental e caso feita de forma errada pode levar a erros ou a impossibilidade da análise.
...
## Os pacotes
Para este capítulo utilizaremos o pacote `tidytext` e `quanteda`, ambos estruturais para análise de conteúdo. Inicialmente trabalharemos com o pacote `tidytext`, sendo utilizado muitas vezes como etapa de introdução ao conteúdo, devido sua convergência com o pacote `tidyverse` em estrutura. Em seguida utilizaremos o pacote `quanteda`, que possui mais funcionalidades de [análise](https://quanteda.io/articles/pkgdown/comparison.html). Trabalharemos com os dois pacotes por serem a base essencial no processo de análise, ambos possuindo suas complementariedades.
