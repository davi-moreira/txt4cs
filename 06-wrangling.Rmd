...
<!--- Explicar melhor a transição entre um pacote e outro
- mostrar que os pacotes preservam a estrutura do documento --->
## Os pacotes
Para este capítulo utilizaremos o pacote `tidytext` e `quanteda`, estes estruturais para análise de conteúdo. Inicialmente trabalharemos com o pacote `tidytext`, sendo utilizado muitas vezes como etapa de introdução ao conteúdo, devido sua convergência com o pacote `tidyverse` em estrutura. Em seguida utilizaremos o pacote `quanteda`, que segue uma estrutura própria podendo ou não, dependendo da etapa de análise, estar contida em um formato data frame. Trabalharemos com os dois pacotes por serem base essencial no processo de análise, ambos possuindo suas complementariedades.
...
<!--- revisar texto e colocar base de dados no pacote do livro --->
...
Para termos um gráfico que compare a proporção de palavras utilizadas pelo PSOL e PSDB em relação ao PT, temos que unir as bases tratadas em uma única, medir a frequência dos termos utilizados por autor, e criar uma coluna com os termos/tokens, uma coluna com a frequência utilizada pelo PT, outra contendo a frequência de uso dos termos pelo PSDB e PSOL e por último uma coluna que identifique o partido, entre PSDB e PSOL.
...
<!--- no paragrafo abaixo, indicar a referencia do link: https://quanteda.io/articles/pkgdown/comparison.html --->
...
A função `tokens()` segmenta o texto, permitindo remover espaços em branco e demais separadores de texto automaticamente, para remoção de pontuação e números é necessário escrever o argumento. Usaremos o corpus gerado do item anterior.  
