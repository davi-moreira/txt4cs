...
```{r, echo = TRUE, eval = F, warning = F, message = F, max.height = "300px"}
library(textreadr)
...
```{r, echo = TRUE, eval = T, warning = F, message = F, max.height = "300px"}
# pacotes 
library(httr)
library(XML)
library(xml2)
...
```{r, echo = TRUE, eval = T, warning = F, message = F, results=F, max.height = "300px"}
bdDados <- bd
...
devtools::install_github("davi-moreira/txt4cs-pkg", force = TRUE)
library(txt4cs)
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, max.height = "100px"}
library(kableExtra)

bd %>% select(nome, partido, uf, discursoPlainTxt) %>% 
  knitr::kable() %>%  kable_styling(bootstrap_options = c("striped", "hover")) %>%
